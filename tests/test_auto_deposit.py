...
def test_auto_drop_not_triggered_before_delay():
    ...
    arrived_time = datetime.utcnow() - timedelta(minutes=5)
    svc._auto_drop_timers[m.id] = arrived_time
    result = svc.check_auto_drop(m.id, current_time=datetime.utcnow())
...
def test_auto_drop_triggered_after_delay():
    ...
    arrived_time = datetime.utcnow() - timedelta(minutes=25)
    svc._auto_drop_timers[m.id] = arrived_time
    result = svc.check_auto_drop(m.id, current_time=datetime.utcnow())
...
def test_auto_drop_records_automatic_flag():
    ...
    arrived_time = datetime.utcnow() - timedelta(minutes=25)
    svc._auto_drop_timers[m.id] = arrived_time
    svc.check_auto_drop(m.id, current_time=datetime.utcnow())
    auto_event = next(
